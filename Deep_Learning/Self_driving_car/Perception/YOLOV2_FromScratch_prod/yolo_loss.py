...
def calc_IOU_pred_true_best(pred_box_xy,pred_box_wh,true_boxes):
    ...
    iou_scores  =  get_intersect_area(true_xy,true_wh,pred_xy,pred_wh) # (N batch, N grid_h, N grid_w, N anchor, 50)
...
#def custom_loss(y_true, y_pred, GRID_W, GRID_H, BATCH_SIZE, BOX, ANCHORS):
def custom_loss(y_true, y_pred):
    ...
    shape_true_box = (BATCH_SIZE, 1, 1, 1, TRUE_BOX_BUFFER, BOX)
    true_boxes = tf.Variable(np.zeros(shape_true_box), dtype="float32")
